...
(0, config_1.task)('deploy:arbTest', 'Deploys Optimism contracts').setAction(async function (taskArguments, { ethers }) {
    const mainnet = false;
    ...
    await hre.run('verify:verify', {
        address: velo.address,
        contract: 'contracts/Velo.sol:Velo',
    ...
    const receiver = await RedemptionReceiver.deploy(OP_CONFIG.USDC, velo.address, FTM_CONFIG.lzChainId, OP_CONFIG.lzEndpoint);
    await receiver.deployed();
    console.log('RedemptionReceiver deployed to: ', receiver.address);
    console.log('Args: ', OP_CONFIG.USDC, velo.address, FTM_CONFIG.lzChainId, OP_CONFIG.lzEndpoint, '\n');
