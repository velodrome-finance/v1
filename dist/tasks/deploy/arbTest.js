...
const testOptimismConfig_1 = __importDefault(
  require('./constants/testOptimismConfig')
)
...
;(0, config_1.task)('deploy:arbTest', 'Deploys Optimism contracts').setAction(
  async function (taskArguments, { ethers }) {
    const mainnet = false
    const OP_CONFIG = mainnet
      ? optimismConfig_1.default
      : testOptimismConfig_1.default
    const FTM_CONFIG = mainnet
      ? fantomConfig_1.default
      : testFantomConfig_1.default
    // Load
    const [
      Flow,
      GaugeFactory,
    ...
    await hre.run('verify:verify', {
      address: flow.address,
      contract: 'contracts/Flow.sol:Flow',
