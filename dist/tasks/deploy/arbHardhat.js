...
const arbHardhatConfig_1 = __importDefault(require("./constants/arbHardhatConfig"));
...
(0, config_1.task)('deploy:arbHardhat', 'Deploys Arbitrum forked chain contracts').setAction(async function (taskArguments, { ethers }) {
    const mainnet = false;
    const ARB_CONFIG = mainnet ? arbConfig_1.default : arbHardhatConfig_1.default;
    const FTM_CONFIG = mainnet ? fantomConfig_1.default : testFantomConfig_1.default;
    console.log('ALCHEMY_MAINNET_ARBITRUM_API_KEY: ', process.env.ALCHEMY_MAINNET_ARBITRUM_API_KEY);
    // Load
    const [Velo, GaugeFactory, BribeFactory, PairFactory, Router, Library, VeArtProxy, VotingEscrow, RewardsDistributor, Voter, Minter, VeloGovernor, RedemptionReceiver, MerkleClaim] = await Promise.all([
    ...
    const receiver = await RedemptionReceiver.deploy(ARB_CONFIG.USDC, velo.address, FTM_CONFIG.lzChainId, ARB_CONFIG.lzEndpoint);
    await receiver.deployed();
    console.log('RedemptionReceiver deployed to: ', receiver.address);
    console.log('Args: ', ARB_CONFIG.USDC, velo.address, FTM_CONFIG.lzChainId, ARB_CONFIG.lzEndpoint, '\n');
    ...
    await minter.initialize(ARB_CONFIG.partnerAddrs, ARB_CONFIG.partnerAmts, ARB_CONFIG.partnerMax);
    console.log('veVELO distributed');
    await minter.setTeam(ARB_CONFIG.teamMultisig);
    console.log('Team set for minter');
    console.log('Arbitrum Forked Chain contracts deployed');
});
