...
function get_y (xIn, a, b) {
  ...
  _c1 = MathDunks.sqrt(c0 * c0 + 108 * x ** 12) + c0
  console.log(_c1)
  c1 = MathDunks.cbrt(_c1)
  b1 = 3 * 2 ** (1 / 3) * x
  b1 = 3 * MathDunks.cbrt(2) * x
  b2 = 2 ** (1 / 3) * x ** 3
  console.log('c1', c1)
  console.log('b1', b1)
  console.log('b2', b2)
...
/*function get_y(xIn, a, b) {
  x = xIn + a;
  y0 = MathDunks.cbrt(MathDunks.sqrt(((27*(a**3)*b*(x**2)+27*a*(b**3)*(x**2))**2)+108*x**12)+27*(a**3)*b*(x**2)+27*a*(b**3)*(x**2))
  y1 = y0 / 3*MathDunks.cbrt(2)*x
  y2 = MathDunks.cbrt(2)*(x**3) / y0
  y = y1 - y2
  console.log(y)
  console.log(b-y)
}*/
...
function getAmountOut (xIn, x, y) {
  ...
  _kB = MathDunks.sqrt(MathDunks.sqrt(_k(x, y))) * 2
  console.log('_kB', _kB)
  _kA1 = MathDunks.sqrt(MathDunks.sqrt(_k(x + xIn, y))) * 2
  yOutAbove = _kA1 - _kB

  _kA2 = MathDunks.sqrt(MathDunks.sqrt(_k(x + xIn, y - yOutAbove))) * 2
  ...
  while (_kA2 > _kB) {
    ...
    _kA2 = MathDunks.sqrt(MathDunks.sqrt(_k(x + xIn, y - yOutBelow))) * 2
  }

  console.log(yOutAbove)
  console.log(yOutBelow)
  yOut = (yOutBelow + yOutAbove) / 2
  _kA2 = MathDunks.sqrt(MathDunks.sqrt(_k(x + xIn, y - yOut))) * 2
  /*while (_kA2 != _kB) {
    diff = _kA2 - _kB;
    yOut = (yOutBelow+yOutAbove)/2;
    console.log(yOut);
